...
root = Tk()
root.title("Faculdade Novos Horizontes")
width = 900
height = 400
...
def database():
    conn = sqlite3.connect("Trabalho_Thiagoav2.py")
    cursor = conn.cursor()
    query = '''CREATE TABLE IF NOT EXISTS Estudante(
    ...
    cursor.execute("SELECT * FROM 'Estudante' ORDER BY nome")
...
def submit_data():
    if nome.get() == "" or email.get() == "" or endereco.get() == "":
        ...
        conn = sqlite3.connect("BancoPython/Trabalho_AV2/faculdade.db")
        cursor = conn.cursor()
        query = '''INSERT INTO Estudante (nome, av1, av2, av3, avd, avds, email, endereco, campus, periodo)  
                        VALUES (?,?,?,?,?,?,?,?,?,?)'''
...
def update_data():
    ...
    conn = sqlite3.connect("BancoPython/Trabalho_AV2/faculdade.db")
    cursor = conn.cursor()
    cursor.execute('''UPDATE Estudante set nome = ?, av1 = ?,  av2 = ?,  av3 = ?,  avd = ?,  avds = ?, email = ?, endereco = ?, campus = ?, periodo = ? WHERE matricula = ?''',
...
def on_select(event):
    ...
    lbl_title = Label(form_titulo, text="Atualizando alunos",
                      font=('arial', 18), bg='7FFFD4', width=280)
...
def delete_data():
    if not tree.selection():
        ...
        if resultado == 'yes':
            ...
            conn = sqlite3.connect("BancoPython/Trabalho_AV2/faculdade.db")
            cursor = conn.cursor()
            cursor.execute("DELETE FROM 'Estudante' WHERE matricula = %d" %
                           selected_item[0])
            conn.commit()
            cursor.close()
            conn.close()
...
def insert_data():
    ...
    lbl_title = Label(form_titulo, text="Novo aluno",
                      font=('arial', 18), bg='#E6E6FA', width=280)
    lbl_title.pack(fill=X)
...
top.pack(side=TOP)
mid = Frame(root, width=500, bg='#E6E6FA')
mid.pack(side=TOP)
midleft = Frame(mid, width=100)
midleft.pack(side=LEFT, pady=10)
midleftPadding = Frame(mid, width=350, bg="#E6E6FA")
...
lbl_title = Label(top, text="Faculdade Novos Horizontes", font=('arial', 18), width=500)
lbl_title.pack(fill=X)
...
bttn_add = Button(midleft, text="Inserir", bg="#6B8E23", command=insert_data)
bttn_add.pack()
